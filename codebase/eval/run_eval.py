...
# Add ROOT to sys.path so we can import agent and tools
...
def validate_case_failure_types(cases: list[dict[str, Any]], path: Path) -> None:
    ...
    for case in cases:
        ...
        if failure_type and failure_type not in ALLOWED_CASE_FAILURE_TYPES:
...
def load_dataset_info(path: Path) -> dict[str, Any]:
    try:
        ...
        if isinstance(data, list):
            return {"dataset_id": path.stem, "dataset_role": "eval", "description": "Tutor Agent evaluation cases"}
        return {
        ...
        return {"dataset_id": path.stem, "dataset_role": "", "description": ""}
...
def compare_subset(expected: dict[str, Any], actual: dict[str, Any]) -> tuple[bool, list[str], int, int]:
    ...
    for key, expected_value in expected.items():
        ...
        if key == "missing_fields":
            ...
            if key in {"query", "reason"} and isinstance(actual_norm, str) and isinstance(expected_norm, str):
                a_clean = actual_norm.replace(" là gì", "").replace("?", "").strip()
                e_clean = expected_norm.replace(" là gì", "").replace("?", "").strip()
                ok = e_clean in a_clean or a_clean in e_clean
            else:
...
def evaluate_phase_b(case: dict[str, Any], tool_calls: list[dict[str, Any]], text: str | None) -> dict[str, Any]:
    expect = case["expect"]
    case_failure_type = case.get("failure_type", "unknown")
...
def print_table(results: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    for item in results:
        ...
        print(f"{str(item['id']):<28} {status:<5} {failure}")
    print()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Tutor Agent live evals.")
    ...
    parser.add_argument("--eval-cases", type=Path, default=DATA_DIR / "golden_set.json")
    ...
    for case in cases:
        ...
        agent = TutorAgent(provider, system_prompt=system_prompt, tools=openai_tools, model=args.model)
        try:
