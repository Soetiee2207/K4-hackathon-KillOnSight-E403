...
def tool_results_message(events: list[dict[str, Any]]) -> dict[str, str]:
    return {
        ...
        "content": (
            ...
            "Hãy sử dụng kết quả các công cụ này để đưa ra câu trả lời cuối cùng cho học viên. "
            "Nhớ đính kèm trích dẫn nguồn cụ thể như [Trang N] hoặc [Txx-NNN] khi trả lời."
        ),
    }
...
def assistant_tool_message(response_text: str | None, calls: list[ToolCall]) -> dict[str, str]:
    ...
    content = response_text or "Tôi sẽ thực thi các công cụ đã chọn."
    return {
        "role": "assistant",
...
def run_model_tool_loop(
    *,
    ...
    for round_index in range(1, max_tool_rounds + 1):
        ...
        for call in calls:
            ...
            if isinstance(result, dict) and result.get("awaiting_user"):
                question = result.get("question") or call.args.get("question") or "Bạn vui lòng bổ sung thông tin nhé."
                rounds.append(round_record)
                return {
    ...
    return {
        ...
        "assistant_text": f"Dừng lại sau {max_tool_rounds} lượt gọi công cụ. Hãy kiểm tra lại transcript để biết chi tiết.",
        "rounds": rounds,
        "tool_events": all_tool_events,
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Interactive Tutor Agent chat with transcript logging.")
    parser.add_argument("--provider", choices=["openrouter", "openai", "anthropic", "gemini", "groq"], required=True)
    parser.add_argument("--model", default=None)
    parser.add_argument("--version", required=True, help="Tên phiên bản, ví dụ: v0, v1.")
    ...
    print(f"VLearn Tutor Agent chat. artifact_version={artifact_version.artifact_version}")
